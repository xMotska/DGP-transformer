...
def autoregressive_sample(
    ...
    # Start with start_element and context
    if np.isscalar(start_element):
        start_symbol = np.full((batch_size, 1), start_element, dtype=context.dtype)
    else:
        start_symbol = np.asarray(start_element)
        if start_symbol.ndim == 1:
            start_symbol = start_symbol[:, None]
    
    current_sequence = np.concatenate([start_symbol, context], axis=1)
...
def autoregressive_sample_jit(
    ...
    def scan_fn(carry, step_rng):
        prev_token, vars_state, step_idx = carry
        ...
        return (sample, new_vars, step_idx + 1), sample[:, 0]

    # Generate remaining tokens
    rngs = jax.random.split(rng, horizon_length - 1)
    initial_step_idx = context.shape[1] + 1  # +1 because we already processed first_sample
    _, samples = jax.lax.scan(scan_fn, (first_sample, variables, initial_step_idx), rngs)
