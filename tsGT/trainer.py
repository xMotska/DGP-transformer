...
def train_predictor(
    ...
    if use_serialization:
        ...
        # inp is (batch, seq_len, n_inputs), upsample seq_len dimension
        n_inputs = inp.shape[2]
        dummy_inp = jnp.ones((inp.shape[0], inp.shape[1] * repr_len, n_inputs))
    else:
        dummy_tokens = jnp.ones_like(series)
        # inp is (batch, seq_len, n_inputs)
        dummy_inp = jnp.ones((inp.shape[0], inp.shape[1], inp.shape[2]))
    ...
    def preprocess_batch(batch):
        ...
        # inp is already (batch, seq_len, n_inputs) from the data pipeline
